...
import (
	...
	oldjsonpointer "github.com/progrium/zt100/vnd/github.com/dustin/go-jsonpointer"
)
...
func SetReflect(o interface{}, path string, value interface{}) (perr error) {
	defer func() {
		if p := recover(); p != nil {
			perr = errors.New(p.(string))
		}
	}()
	if path == "" {
		return
...
OUTER:
	...
	if value == nil {
		...
		if rv.Type().Kind() == reflect.Slice {
			nv = reflect.MakeSlice(val.Type(), 0, 0)
			for i := 0; i < rv.Len(); i++ {
				v := rv.Index(i)
				nv = reflect.Append(nv, v.Convert(nv.Type().Elem()))
			}
		} else {
			nv = rv.Convert(val.Type())
		}
		val.Set(nv)
	}
	return
